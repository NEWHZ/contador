...
function openTimerModal(spaceId) {
    ...
    if (storedStartTime !== null) {
        ...
        if (!isPaused) {
            ...
            document.getElementById("startPauseBtn").classList.remove("btn-success", "btn-warning");
...
function startPauseTimer() {
    ...
    if (!isStopwatch) {
        ...
        const remainingTime =
            countdownTime -
            Math.floor(
                (new Date().getTime() - parseInt(localStorage.getItem(`startTime_${currentSpaceId}`))) / 1000
            );
...
function syncTimerData() {
    ...
    // Calcular el tiempo restante en el countdown
    const startTime = parseInt(localStorage.getItem(`startTime_${currentSpaceId}`));
    if (!startTime) return;
    ...
    const remainingTime = countdownTime - elapsedTime; // Tiempo restante en segundos

    // Si el tiempo restante es mayor a cero, proceder con la sincronización
    if (remainingTime > 0) {
        registrarAlquiler(currentSpaceId, remainingTime)
            .then((response) => {
...
function calculateAndDisplayTime() {
    ...
    if (isStopwatch) {
        ...
        timeElapsed = parseInt(localStorage.getItem(`timeElapsed_${currentSpaceId}`)) + elapsedTime;
...
function updateDisplay() {
    if (isStopwatch) {
        ...
        const remainingTime =
            parseInt(localStorage.getItem(`countdownTime_${currentSpaceId}`)) || countdownTime;
        document.getElementById("timer-display").textContent = formatTime(remainingTime);
    }
}
